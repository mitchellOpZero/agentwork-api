...
Use `/catalog` to browse verified listings free. For structured automation, start with `/v1/manifest`, then use `/v1/quote` to inspect filtered count and payout ranges without exposing listings. Treat `/v1/feed` and `/v1/opportunities/{id}` as paid routes. Never spend without the user's permission or an existing wallet policy that covers the live challenge (currently 0.005 USDC for 60 minutes).
